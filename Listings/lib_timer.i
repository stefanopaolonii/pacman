...
uint32_t init_timer ( uint8_t timer_num, uint32_t Prescaler, uint8_t MatchReg, uint8_t SRImatchReg, uint32_t TimerInterval )
{
 ...
 __NVIC_SetPriority(TIMER0_IRQn, 8);
 return (0);
  }
  else if ( timer_num == 1 )
  {
...
# 261 "Source/timer/lib_timer.c"
uint32_t sec_to_count(float sec) {
    return (uint32_t)(sec * 25000000);
}
# 275 "Source/timer/lib_timer.c"
float count_to_sec(uint32_t count) {
    return (float)count / (float) 25000000;
}
# 289 "Source/timer/lib_timer.c"
int timer_not_done(uint8_t timer_num) {
 if ( timer_num == 0 ){
  return (((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x04000) )->TCR & (1 << 0));
   }else if (timer_num == 1){
  return (((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x08000) )->TCR & (1 << 0));
   }else if (timer_num == 2){
  return (((LPC_TIM_TypeDef *) ((0x40080000UL) + 0x10000) )->TCR & (1 << 0));
 }else if(timer_num ==3){
  return (((LPC_TIM_TypeDef *) ((0x40080000UL) + 0x14000) )->TCR & (1 << 0));
 }else{
     return 0;
 }
}
# 313 "Source/timer/lib_timer.c"
void timer_turnon(){
