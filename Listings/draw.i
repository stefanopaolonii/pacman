# 1 "Source/Draw/draw.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 393 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "Source/Draw/draw.c" 2
...
void draw_life(int life_x, int life_y, int lifes){
    int i;
    for(i=0;i<3;i++){
        draw_square(0,i,life_x,life_y,9,0x0000);
    }
    for(i=0;i<lifes;i++){
...
void draw_timer(uint16_t pos_x, uint16_t pos_y,uint16_t text_color, uint16_t bg_color, int time){
    ...
    GUI_Text(210, 0, (uint8_t *)tmp, 0xFFFF, 0x0000);
...
void draw_pacman(int mat_x, int mat_y,int map_x, int map_y, int direction){
    DrawPacman((mat_y*9)+4+ map_x,(mat_x*9)+4+map_y,6,direction);
}

void draw_ghost(int mat_x, int mat_y,int map_x, int map_y, int color){
    DrawGhost((mat_y*9)+4+ map_x,(mat_x*9)+4+map_y,9,color);
}
