# 1 "Source/Astar/Astar.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 393 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "Source/Astar/Astar.c" 2
# 1 "Source/Astar\\Astar.h" 1
...
# 2 "Source/Astar/Astar.c" 2

extern char map[31][26];
extern GHOST_STATE ghost_status;

int visited[31][26]; // Array to keep track of visited nodes
Node allNodes[(31*26)]; // Array to store all nodes
int allNodesCount = 0; // Counter for the next available node
Node* openList[(31*26)]; // List of nodes to explore
...
int aisValid(int x, int y) {
    return (x >= 0 && x < 31 && y >= 0 && y < 26 && map[x][y] != '#');
}
